...
export default function ProductDetailPage() {
  ...
  // This returns your addItem method which should handle single seller cart enforcement
  const addItem = useCartStore((state) => state.addItem)
  const items = useCartStore((state) => state.items) // get current cart items for seller check
  ...
  const handleAddToCart = () => {
    ...
    if (items.length > 0 && product.sellerId !== items[0].sellerId) {
      // Show error if sellerId of new product does not match existing cart items' sellerId
      toast.error(
        'Your cart contains items from another seller. Please clear your cart before adding products from a different seller.'
      )
      return
    }

    // Proceed to add item if seller is consistent
    if(addItem(product, quantity))
    {
      // If addItem returns true, it means item was successfully added
      toast.success(`${product.name} added to cart`)
      router.push('/cart')
    }
    else{
      toast.success(`Your cart already has items from a different seller. Please clear your cart to add products from seller ${product.sellerName || 'Unknown'}.`)
    }
  }
